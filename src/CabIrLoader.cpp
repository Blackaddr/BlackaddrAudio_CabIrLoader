...
{
	if (idx >= (int)NUM_IMPULSE_RESPONSES) { return false; }
...
void CabIrLoader::cabselect(float value)
{
	...
	if (!m_config(newIdx)) {
		EFX_PRINT(Serial.printf("CabIrLoader(): invalid cab select: %df\n\r", newIdx));
	}
	EFX_PRINT(Serial.printf("CabIrLoader(): Requested cab: %d cab selected: %d\n\r", requestedCab, newIdx));
}
